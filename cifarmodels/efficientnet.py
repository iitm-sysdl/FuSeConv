...
class BlockFriendly(nn.Module):
    ...
    def __init__(self,
                 ...
                 se_ratio=0.,
                 drop_rate=0.):
        super(BlockFriendly, self).__init__()
        self.stride = stride
        self.drop_rate = drop_rate
        self.expand_ratio = expand_ratio
        ...
        padding = 1 if kernel_size == 3 else 2
        self.conv2_h = nn.Conv2d(channels//2, channels//2, kernel_size=(1, kernel_size), stride=stride, padding=(0, padding), groups=channels//2, bias=False)
        self.conv2_d = nn.Conv2d(channels//2, channels//2, kernel_size=(kernel_size, 1), stride=stride, padding=(padding, 0), groups=channels//2, bias=False)
        self.bn2 = nn.BatchNorm2d(channels)
    ...
    def forward(self, x):
        ...
        out = torch.cat([self.conv2_h(out1), self.conv2_d(out2)],1)
        out = swish(self.bn2(out))
        out = self.se(out)
        out = self.bn3(self.conv3(out))
        if self.has_skip:
            if self.training and self.drop_rate > 0:
...
def EfficientNetB0Friendly(num_classes=100):
    return EfficientNet(BlockFriendly, cfg, num_classes)

def test():
    net = EfficientNetB0()
    x = torch.randn(4, 3, 32, 32)
    y = net(x)
    net = EfficientNetB0Friendly()
    x = torch.randn(4, 3, 32, 32)
    y = net(x)
    print(y.shape)
