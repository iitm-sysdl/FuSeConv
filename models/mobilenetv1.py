...
class MobileNetHybrid(nn.Module):
    # (128,2, 1) means conv planes=128, conv stride=2, by default conv stride=1 and Fuse layer
    depth_mul = 1
    cfg = [
            [64,  1, 1],
            [128, 2, 1], 
    ...
    def __init__(self, block, num_classes):
        super(MobileNetHybrid, self).__init__()
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        self.layers = self._make_layers(block, in_planes=32)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
    ...
    def _make_layers(self, block, in_planes):
        layers = []
        for x, s, f in self.cfg:
            out_planes = int(x * self.depth_mul)
            stride = s
            if f ==1:
                blockHybrid = block
            else:
                blockHybrid = Block
            layers.append(blockHybrid(in_planes, out_planes, stride))
            in_planes = out_planes
        return nn.Sequential(*layers)
...
def MobileNetV1FuSeHalf(num_classes=1000):
    return MobileNet(FuSeHalfBlock, num_classes)

def MobileNetV1FuSeFull(num_classes=1000):
    return MobileNet(FuSeFullBlock, num_classes)

def MobileNetV1FuSeHalfHybrid(num_classes=1000):
    return MobileNetHybrid(FuSeHalfBlock, num_classes)

def MobileNetV1FuSeFullHybrid(num_classes=1000):
    return MobileNetHybrid(FuSeFullBlock, num_classes)
...
def test():
    ...
    y = net(x)
    print(y.size())
    net = MobileNetV1FuSeFull()
    x = torch.randn(1,3,224,224)
    y = net(x)
    print(y.size())
    net = MobileNetV1FuSeHalf()
    x = torch.randn(1,3,224,224)
    y = net(x)
    print(y.size())
    net = MobileNetV1FuSeFullHybrid()
    x = torch.randn(1,3,224,224)
    y = net(x)
    print(y.size())
    net = MobileNetV1FuSeHalfHybrid()
    x = torch.randn(1,3,224,224)
    y = net(x)
    print(y.size())
