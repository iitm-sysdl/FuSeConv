...
class BlockFriendly(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn2(torch.cat( (self.conv2_h(out1), self.conv2_v(out2)),1))
        out = self.bn3(self.conv3(out))
        out = out + self.shortcut(x) if self.stride==1 else out
        return out
...
def MobileNetV2(num_classes=100):
    return MobileNet(Block, num_classes)

def MobileNetV2Friendly(num_classes=100):
    return MobileNet(BlockFriendly, num_classes)
