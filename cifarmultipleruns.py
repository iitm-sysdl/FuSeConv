...
def main(trial):
    ...
    if args.resume == True:
        ...
        checkpoint = torch.load(args.name+'/BestModel' + str(trial) + '.t7')
    ...
    for epoch in range(startEpoch, 150):
        ...
        if acc > bestAcc:
            torch.save(state, args.name+'/BestModel' + str(trial) + '.t7')
            bestAcc = acc
            wandb.save('BestModel'+ str(trial) + '.h5')
        else:
            torch.save(state, args.name+'/LastEpoch' + str(trial) + '.t7')
